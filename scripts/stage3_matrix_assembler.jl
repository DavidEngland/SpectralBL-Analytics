...
function assemble_stage3_matrices(
    ...
    # Group statistics by route class for audit summary.
    route_counts = combine(groupby(select(packets, :route_class), :route_class), nrow => :count)
    ...
    for row in eachrow(packets)
        ...
        tau = Int(row.selected_tau)
        route = canonical_route(String(row.route_class))

        rows_here = 0
        if route == "Stationary" || tau <= 0
            rows_here = max(0, win_n - 1)
        else
            rows_here = max(0, (win_n - 2 * tau) - 1)
        end

        push!(local_rows, rows_here)
        push!(route_group, route)
