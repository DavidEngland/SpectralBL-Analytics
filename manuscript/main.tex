...
To provide a mathematical prototype for folded equilibrium geometry, we reference the canonical cusp-catastrophe normal form \citep{thom1975, zeeman1977, poston1978}. For a state variable $x$ and control parameters $(a,b)$,
\begin{equation}
...
When external forcing pushes the system to the edge of the coupled sheet, the upper equilibrium point ceases to exist. This boundary is naturally interpreted as consistent with a saddle-node condition
\begin{equation}
\det(\mat{J}) = 0,
\end{equation}
where $\mat{J}=\partial \mathbf{F}/\partial \mathbf{x}$ denotes the equilibrium Jacobian associated with the continuation equations \citep{derbyshire1999}. The catastrophic transition is therefore not an instability growing infinitely from a persistent state; it is the disappearance of the stable equilibrium the atmosphere was occupying. Forced off this edge, the state trajectory must rapidly relocate to the only available stable alternative: the decoupled branch. This links geometric evolution directly to the physical interpretation of the maximum sustainable heat flux \citep{vandewiel2017, vignon2017}: the fold edge represents the structural limit beyond which no nearby coupled equilibrium can exist, and the crossing of $\Ric$ is interpreted as a local, downstream projection of this limit point.
...
\end{align}
\textbf{Level 5} is operator discovery conditioned on closure outcomes, and \textbf{Level 6} is operational parameterization. Levels 5--6 are therefore development stages contingent on the Level-4 outcome rather than direct evidence levels. The present manuscript substantiates Levels 1--3 directly and reports Level-4 structure as a testable program.

\subsection{Fold-proximal bending diagnostic}

To convert visual fold interpretation into a measurable diagnostic, we define a local fold-bending indicator
\begin{equation}
B_f = \left|\frac{d^2\eta_3}{d\eta_1^2}\right|.
\end{equation}
Fold-proximal regions are then identified as local maxima of $B_f$ along campaign trajectories. Here $B_f$ is used explicitly as a second-derivative diagnostic, not as full geometric curvature in the differential-geometric sense. In this manuscript, $B_f$ serves as an empirical marker for transition-prone neighborhoods rather than as proof of a unique canonical catastrophe model.
...
\end{tabularx}
\vspace{2pt}
{\footnotesize\emph{Note: Quantitative values will be populated following completion of the unified rerun across all campaigns.}}
\end{table}
...
\toprule
Campaign & Rigid scale only & Rigid rotation ($\theta$) & Bounded affine & Fixed-rigid $\mu$-only \\
\midrule
