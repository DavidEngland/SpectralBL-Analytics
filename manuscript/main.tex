...
\end{equation}
is exactly what the p-FEM mass matrix $\mat{M}$ preserves, so the decomposition is performed in the $\mat{M}$-weighted Hilbert-space inner product $\langle u,v\rangle_M$. In this Hilbert-space formulation, the leading coordinates $\eta_k$ are mesh-invariant coordinates of the atmospheric column: they are invariant to sensor clustering and height spacing and depend only on physical structure overlap. This is the same mathematical class of weighted POD construction used in Hilbert-space formulations \citep{fareed2018}. The SVD basis $\{\vect{v}_k\}$ is $\mat{M}$-orthonormal:
\begin{equation}
