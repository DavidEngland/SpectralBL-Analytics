...
Campaign/site context from the curated dataset catalog identifies \texttt{ FLOSS / FLOSS II } as a \texttt{ Marginal Stability Transition } case with status \texttt{ Ingested } and ingestion path data/floss; FLOSS II materials provided to framework by Larry Mahrt.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l p{9cm}}
        \toprule
        \textbf{Diagnostic Metric} & \textbf{Pipeline Evaluation Summary for \texttt{ FLOSS }} \\
        \midrule
        Data Footprint & Analyzed 70796 observations across a temporal scale of 11732.5 hours. \\[4pt]
        Manifold Complexity & Mean singular value entropy registers at $H_{\text{mean}} = 0.425$, yielding a Shannon effective dimension of $D_{\text{eff}} = 1.4920$. \\[4pt]
        Numerical Stability & The condition number $\kappa = 55.88$ confirms a well-conditioned low-rank basis projection ($N_0 = 0$ nullspace modes). \\
        \bottomrule
    \end{tabular}
...
\begin{equation}
    \mathcal{M}_{\text{validation}} = \mathbf{f}\Big(\mathcal{A}_{\text{site}}\big(\kappa_{\text{rank}}=55.88,\ D_{\text{eff}}=1.4920\big),\ \mathcal{S}_{\text{surface}}\Big)
\end{equation}

Surface roughness context for this run: \texttt{ 0.000 m }.
...
For this campaign, the catalog maps run metadata to dataset entry \texttt{ FLOSS / FLOSS II } with status \texttt{ Ingested }, priority \texttt{ core }, and archetype \texttt{ Marginal Stability Transition }.

Primary reference set: Sun et al. (2020); Mahrt and Vickers (2006); Acevedo et al. (2021).

Ingestion/source note: data/floss; FLOSS II materials provided to framework by Larry Mahrt.
