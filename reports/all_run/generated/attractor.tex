...
Figure~\ref{fig:attractor_CASES-99} visualizes the three-dimensional trajectory $(\eta_1, \eta_2, \eta_3)$ colored by the Singular Value Entropy diagnostic $H(t)$. Each point represents a timestep in the boundary layer evolution, with the color gradient indicating the effective dimensionality and structural complexity at that moment.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            colorbar,
            point meta=explicit,
            colorbar style={title={$H(t)$}, title style={yshift=5pt}},
            mark size=1.6pt,
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{3D attractor state trajectory colored by singular value entropy.}
...
\begin{itemize}
    ...
    \item \textbf{Color evolution by $H(t)$} indicates structural order: higher entropy marks distributed multi-mode activity; lower entropy marks collapsed, strongly stratified states.
\end{itemize}
...
Operationally, peaks in $\|\boldsymbol{\eta}\|$ often coincide with rapid boundary-layer reconfiguration, while entropy excursions identify windows where similarity-based single-regime assumptions are least reliable.
...
The entropy-derived $D_{\text{eff}}$ quantifies how many modes are actively carrying structure in practice, rather than only counting nominal basis size. Larger values indicate broader modal participation and lower compressibility of the observed boundary-layer state.
