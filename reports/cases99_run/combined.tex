...
The boundary layer is analyzed as a low-dimensional dynamical system through projection onto dominant singular vectors. The following section presents the reconstructed phase space trajectory for campaign \texttt{ CASES-99 } using the v0.1 baseline.

\begin{description}
    \item[Campaign:] CASES-99
    \item[Baseline Version:] v0.1
    \item[Total Samples:] 6538
    \item[Mean Entropy:] $H_{\text{mean}} = 0.211$
\end{description}

\subsection{3D Phase Space Projection}

Figure~\ref{fig:attractor_CASES-99} visualizes the three-dimensional trajectory $(\eta_1, \eta_2, \eta_3)$. To highlight the true multi-regime orbital evolution, timesteps are connected sequentially by a continuous historical path line. The trajectory is colored using a high-contrast colormap driven by the \textbf{Attractor Spin} diagnostic ($\Omega(t)$), explicitly isolating the directionality of regime transitions.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            ylabel={$\eta_2$},
            zlabel={$\eta_3$},
            title={Low-Rank Attractor Trajectory (\texttt{ CASES-99 })},
            colormap/jet,
            colorbar,
            point meta=explicit,
...
\addplot3[
                ...
                z=eta_3,
                meta=attractor_spin
            ] {../../data/outputs/regime_scatterplots_cases_99.csv};
...
\begin{align}
    \langle \eta_1 \rangle &= 0.583 \\
    \langle \eta_2 \rangle &= 0.289 \\
    \langle \eta_3 \rangle &= -0.080
\end{align}
...
\begin{itemize}
    \item \textbf{Campaign:} CASES-99
    \item \textbf{Baseline:} v0.1 (spectralbl-attractor)
    \item \textbf{Temporal Coverage:} 685.9 hours
    \item \textbf{Sample Count:} 6538 observations
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            xlabel={$\eta_1$},
            ylabel={$\eta_2$},
            title={Spectral Orthogonality Projection (CASES-99)},
            colormap/plasma,
            colorbar,
            point meta=explicit,
    ...
    \label{fig:spectral_orthogonality_cases-99}
\end{figure}
...
\begin{itemize}
    \item \textbf{ Attractor Collapse Identification: } Low mean entropy and compressed effective dimension indicate recurrent attractor collapse into highly ordered nocturnal states.
    \item \textbf{ Dominance of Non-Local Regimes: } The decomposition highlights intermittent shear-burst behavior and non-local coupling episodes that challenge strictly local similarity formulations.
    \item \textbf{ Transition Behavior Tracking: } Phase-space coordinates remain informative in stability windows where Richardson-style diagnostics saturate and lose structural sensitivity.
\end{itemize}

\subsection{Operational Framework Recommendation}

These signatures confirm that \texttt{ CASES-99 } contributes campaign-specific constraints in the verification pipeline. Its operational role can be summarized through runtime geometry and conditioning diagnostics:
\begin{equation}
    \mathcal{M}_{\text{validation}} = \mathbf{f}\Big(\mathcal{A}_{\text{site}}\big(\kappa_{\text{rank}}=6.85,\ D_{\text{eff}}=2.0537\big),\ \mathcal{S}_{\text{surface}}\Big)
\end{equation}

Surface roughness context for this run: \texttt{ 0.030 m }.
